...
def create_bdx():
    with sqlite3.connect(path_to_db) as db:
        ...
        if len(check_create_purchases) == 15:
            ...
            with sqlite3.connect(path_to_db) as db:
                db.execute("CREATE TABLE storage_adminez("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT, "
                        "user_id INTEGER)")
            print("DB adminez | Создание DB...")
        db.commit()
create_bdx()
...
def add_adminez(user_id):
    with sqlite3.connect(path_to_db) as db:
        db.execute("INSERT INTO storage_adminez "
                   "(user_id) "
                   "VALUES (?)",
                   [user_id])
        db.commit()
def get_adminez(**kwargs):
    with sqlite3.connect(path_to_db) as db:
        sql = "SELECT * FROM storage_adminez WHERE "
